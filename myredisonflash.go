...
func handleGetStat(w http.ResponseWriter, r *http.Request) {
	postData, _ := ioutil.ReadAll(r.Body)
	queryParams, _ := url.ParseQuery(string(postData))
	fmt.Println(queryParams)
	...
	value_start_uint64, value_err := strconv.ParseUint(value_start[0], 10, 64)
	if value_err != nil {
		w.WriteHeader(http.StatusBadRequest) // BadRequest
		return
	}

	value_end_uint64, value_err := strconv.ParseUint(value_end[0], 10, 64)
	if value_err != nil {
		w.WriteHeader(http.StatusBadRequest) // BadRequest
		return
	}
